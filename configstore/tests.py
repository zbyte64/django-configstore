...
from forms import ConfigurationForm, EncryptedConfigurationForm
...
class TestEncryptedConfigurationForm(EncryptedConfigurationForm):
    setting1 = forms.CharField()
    setting2 = forms.IntegerField()

class TestComplexEncryptedConfigurationForm(EncryptedConfigurationForm):
    amount = forms.DecimalField()
    user = forms.ModelChoiceField(queryset=User.objects.all())

class EncryptedConfigStoreTest(TestCase):
    # CONSIDER: There are no views to test do we need this?
    # urls = 'configstore.test_urls'
    ...
    def setUp(self):
        ...
        if hasattr(CONFIG_CACHE, 'testcomplex'):
            delattr(CONFIG_CACHE, 'testcomplex')
        self.instance = ConfigurationInstance('test', 'test', TestEncryptedConfigurationForm)
        register(self.instance)
        self.complex_instance = ConfigurationInstance('testcomplex', 'testcomplex', TestComplexEncryptedConfigurationForm)
        register(self.complex_instance)
    ...
    def test_is_encrypted(self):
        ...
        self.assertNotEqual(0, len(lazydictionary_post.items()))
        conf = Configuration.objects.get(key='testcomplex')
        decoded_data = base64.b64decode(conf._data)
        self.assertFalse('5.00' in decoded_data)
