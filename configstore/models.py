...
class Configuration(models.Model):
    ...
    def encrypt_data(self, value):
        enc = AES.new(settings.SECRET_KEY[:32], AES.MODE_ECB)  # Guess why :32? (32*8=256 bytes for the key)
        value = enc.encrypt(self.pad_string(value, AES.block_size))
        return base64.b64encode(value)

    def decrypt_data(self, value):
        value = base64.b64decode(value)
        dec = AES.new(settings.SECRET_KEY[:32], AES.MODE_ECB)  # Guess why :32? (32*8=256 bytes for the key)
